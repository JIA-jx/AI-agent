...
type PostgresLog struct {
	...
	LogLevel                  *string        `mapstructure:"level"`
	IgnoreRecordNotFoundError *bool          `mapstructure:"ignoreRecordNotFoundError"`
	ParameterizedQueries      *bool          `mapstructure:"parameterizedQueries"` // 是否在日志中显示参数化查询
	Colorful                  *bool          `mapstructure:"colorful"`
}
...
func Init() *viper.Viper {
	var configFile = pflag.StringP("config",
		"c",
		"etc/config.yml",
		"Path to the config file (e.g., etc/config.yml)",
	)
	pflag.Parse()
