...
func (t *QwenVLNode) processImage(url string) (string, error) {
	...
	contentType := http.DetectContentType(data)                              // 类型
	encodeToString := base64.StdEncoding.EncodeToString(data)                // base64 解析数据
	dataURL := fmt.Sprintf("data:%s;base64,%s", contentType, encodeToString) // 请求构造
	return dataURL, nil
}
